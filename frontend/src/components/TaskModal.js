...
export default function TaskModal({
  ...
  const handleStatusChange = async (newStatus) => {
    ...
    if (isEdit) {
      ...
      } catch { /* handled on form submit if needed */ }
    }
  };
      ...
      <div className="modal modal-wide">
        ...
        {isEdit && (
          <div className="tab-bar" style={{ padding: '0 24px', marginBottom: 0 }}>
            {[
              ['details', 'Details'],
              ['comments', `Comments${taskData?.comments?.length ? ` (${taskData.comments.length})` : ''}`],
            ].map(([v, l]) => (
              <button
                key={v}
          ...
          <div className="modal-body">
            ...
            {taskData?.comments?.map((c) => (
              <div key={c._id} className="comment">
                <div
                  className="avatar"
                  style={{ background: avatarColor(c.author?.name), flexShrink: 0 }}
                >
