...
const PROJECT_COLORS = ['#cba6f7','#89b4fa','#94e2d5','#a6e3a1','#f9e2af','#fab387','#f38ba8','#74c7ec'];
      ...
      <div className="page-content">
        ...
        <div className="grid-4 mb-4">
          {['all','active','on-hold','completed'].map(s => {
            ...
            const labels = { all: 'Total Projects', active: 'Active', 'on-hold': 'On Hold', completed: 'Completed' };
            const colors = { all: 'accent', active: 'green', 'on-hold': 'yellow', completed: 'blue' };
            return (
              <div key={s} className="card stat-card">
                <div className="stat-label">{labels[s]}</div>
                <div className={`stat-value ${colors[s]}`}>{count}</div>
              </div>
            );
          })}
        </div>
        ...
        {filtered.length === 0 ? (
          <div className="empty-state">
            ...
            <div className="empty-state-title">{search || filter !== 'all' ? 'No matching projects' : 'No projects yet'}</div>
            ...
            {filtered.map(p => (
              <div key={p._id} className="card project-card" style={{ '--color': p.color || '#cba6f7' }}>
                <div className="flex items-center justify-between mb-3">
                  ...
                  <div className="flex gap-2">
                    <button className="btn btn-ghost btn-sm btn-icon" title="Edit" onClick={() => { setEditProject(p); setShowModal(true); }}>✏️</button>
                    <button className="btn btn-ghost btn-sm btn-icon" title="Delete" onClick={() => handleDelete(p._id)}>🗑️</button>
                  </div>
                </div>
                ...
                <div className="project-card-footer">
                  ...
                  <div className="member-stack">
                    {[p.owner, ...(p.members || [])].slice(0, 4).map((m, i) => m && (
                      <div key={m._id || i} className="avatar" title={m.name} style={{ background: `hsl(${m.name?.charCodeAt(0) * 15 || 200}, 60%, 45%)` }}>
                        {getInitials(m.name)}
                      </div>
                    ))}
                  </div>
