...
export default function Dashboard() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        // Load projects first so we can load per-project tasks for the progress bars
        ...
        // For progress bars: load tasks for active projects only
  ...
  const overdueTasks = myTasks.filter(t => t.dueDate && isBefore(new Date(t.dueDate), new Date()) && t.status !== 'done');
  const upcomingTasks = myTasks.filter(t => t.dueDate && isAfter(new Date(t.dueDate), new Date()) && isBefore(new Date(t.dueDate), addDays(new Date(), 7)) && t.status !== 'done');
    ...
    <div>
      ...
      <div className="page-content">
        ...
        <div style={{ marginBottom: 28 }}>
          <h1 style={{ fontSize: '1.6rem', fontWeight: 700 }}>Good {getGreeting()}, {user?.name?.split(' ')[0]} 👋</h1>
        ...
        <div className="grid-4 mb-4">
          ...
          <div className="card stat-card">
            ...
            <div className="stat-value" style={{ color: overdueTasks.length > 0 ? 'var(--red)' : 'var(--green)' }}>{overdueTasks.length}</div>
            ...
            {activeProjects.length === 0 ? (
              <div className="card">
                <div className="empty-state" style={{ padding: '30px 20px' }}>
                  ...
                  <div className="empty-state-desc">Create a project and set it to active to see it here.</div>
                </div>
              </div>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                {activeProjects.slice(0, 5).map(p => {
                  const pTasks = allTasks.filter(t => {
                    const tProject = typeof t.project === 'object' ? t.project._id : t.project;
                    return String(tProject) === String(p._id);
                  });
